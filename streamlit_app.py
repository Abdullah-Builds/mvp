...
.footer {
    ...
    /* Make the toggle label prominent */
    div[data-testid="stToggle"] label {
        font-size: 16px !important;
        font-weight: 700 !important;
    }

    /* Enlarge the complete toggle */
...
# ----------------------------- Groq / AI helpers -----------------------------
# Defined early so both the Prediction page (AI summary, lifestyle plan) and
# the AI Assistance chat page can use them.

GROQ_MODEL = "openai/gpt-oss-20b"

AI_SAFETY_RULES = """
Rules you must always follow:
1. Do not diagnose the patient or claim certainty about a medical condition.
2. Do not prescribe medicines, dosages, or specific treatments.
3. Do not invent patient data or model results beyond what is supplied.
4. Make clear that model predictions are estimates, not clinical conclusions.
5. For potentially urgent symptoms, advise seeking prompt professional
...
def get_ai_language():
    return st.session_state.get("ai_language", "English")


def _prediction_context_block(prediction):
...
def generate_ai_summary(prediction, language="English"):
    """One-paragraph, plain-language explanation of the prediction,
    generated once per prediction and cached in session_state."""
    ...
    system_prompt = f"""
You are the AI summary layer of a cardiac-risk prediction MVP. Explain the
supplied model outputs to the patient in 3-5 short, plain-language sentences.
Mention the risk category and what it broadly implies, without alarming
            ...
            messages=[
                {
                    ...
                    + "\n\nCONTEXT:\n"
                    + _prediction_context_block(prediction),
                },
                {
                    "role": "user",
                    "content": "Summarize this cardiac risk result for the patient.",
                },
            ],
            temperature=0.3,
            max_tokens=350,
...
def generate_lifestyle_plan(prediction, values, language="English"):
    """A more detailed, practical lifestyle/diet/activity plan grounded in
    the patient's own inputs (smoking, activity, sitting, sleep, etc.)."""
    ...
    system_prompt = f"""
You are the AI lifestyle-coaching layer of a cardiac-risk prediction MVP.
Using the model's risk output and the patient's lifestyle inputs, write a
short, practical, encouraging lifestyle plan with clear sections (e.g.
Activity, Diet, Sleep & Stress, Follow-up). Use general, non-prescriptive,
evidence-based guidance only (no specific drug names, doses, or medical
procedures). Keep it under 220 words. Respond in {language}.
{AI_SAFETY_RULES}
"""
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        + "\n\nCONTEXT:\n"
                        + _prediction_context_block(prediction)
                        + f"\n\nLifestyle-related inputs: {lifestyle_fields}"
                    ),
                },
                {
                    "role": "user",
                    "content": "Write my personalized lifestyle plan.",
                },
            ],
            temperature=0.4,
            max_tokens=500,
...
def stream_groq_reply(client, messages):
    """Yields incremental text chunks from a streaming Groq completion."""
...
if "bmi_calculator" not in st.session_state:
    st.session_state.bmi_calculator = None
...
with st.sidebar:
    ...
    if st.button("Calculate BMI", use_container_width=True):
        set_page("Calculate BMI")

    st.markdown('<div class="nav-divider"></div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="nav-label">AI Response Language</div>',
        unsafe_allow_html=True,
    )
    st.selectbox(
        ...
        help="Applies to the AI summary, lifestyle plan, and AI Assistance chat.",
    )
...
if st.session_state.active_page == "Prediction":
    ...
    #
    # This page intentionally does NOT use st.form(). Widgets inside
    # st.form() only report their value when the form is submitted —
    # they don't trigger a rerun on their own. Since the ECG/Lab
    # "Optional" toggle needs to instantly show/hide requirement on
    # the fields right next to it, every widget here is a plain,
    # reactive widget, and "Predict cardiac risk" is a normal button
    # that reads whatever is currently in `values`.
    # ============================================================
    ...
    available_ecg_lab = [field for field in ecg_lab_fields if field in feature_cols]
    ...
    for section_name, fields in sections:
        ...
        st.markdown(
            f'<div class="section-title">' f"<span>✦</span>{section_name}</div>",
            unsafe_allow_html=True,
        )
    ...
    # ECG & LAB SECTION
    #
    # The "Optional" toggle sits right in this section's own header,
    # so switching it instantly flips the fields immediately below
    # between optional (skippable) and required (pre-filled with
    # training medians). Left blank / "Not provided" values are
    # backfilled with training-set defaults at submit time.
    # ========================================================

    if available_ecg_lab:

        st.markdown(
            """
            <style>
            /* ---- Prominent ECG & Lab "Optional" toggle card ---- */
            div[data-testid="stVerticalBlockBorderWrapper"]:has(
                div.st-key-ecg_toggle_card
            ) {
                background: linear-gradient(135deg, #eaf3ff 0%, #dcecff 100%);
                border: 1.5px solid #7fb2f2 !important;
                border-radius: 18px !important;
                box-shadow:
                    0 12px 28px rgba(13, 111, 232, .16),
                    inset 0 1px 0 rgba(255,255,255,.6);
                padding: 4px 6px;
                margin: 6px 0 16px;
            }

            .st-key-ecg_toggle_card .section-title {
                margin: 6px 0 2px;
            }

            /* Make the toggle switch itself bigger and higher-contrast */
            div[data-testid="stToggle"] {
        ...
        with st.container(key="ecg_toggle_card", border=True):
            ...
            with header_col:
                st.markdown(
                    ...
                    "<span>✦</span>ECG & Lab Values"
                    "</div>",
                    unsafe_allow_html=True,
                )

            with toggle_col:
                # Defaults to ON so every field in this section starts
                # out optional. Toggle it off to require them — this
                # updates the fields below immediately.
                ecg_lab_optional = st.toggle(
                    "Optional",
                    value=True,
                    key="ecg_lab_optional",
                )

            st.caption(
                "Leave these blank to use training-set defaults."
                if ecg_lab_optional
                else "These fields are required — pre-filled with training-set medians."
            )

        cols = st.columns(2)

        for index, field in enumerate(available_ecg_lab):
...
if submitted:
    ...
    # If optional ECG/Lab fields were left blank, backfill them with the
    # same training-set defaults used elsewhere in the form so the model
    # always receives a complete feature row.
    if ecg_lab_optional:

        for field in available_ecg_lab:

            if field in numeric_cols:

                if values.get(field) is None:
                    values[field] = numeric_defaults.get(field, 0.0)

            elif values.get(field) == "Not provided":

                options = categorical_options.get(field, [])
                values[field] = options[0] if options else "Unknown"

    # Safety net: fill any other stray blank numeric values with defaults.
    for field in numeric_cols:
        if values.get(field) is None:
            values[field] = numeric_defaults.get(field, 0.0)

    # ----------------------------- Prediction ----------------------------------
    input_df = pd.DataFrame([{column: values.get(column) for column in feature_cols}])

    try:
        score = float(models["framingham_score"].predict(input_df)[0])
        years = float(models["est_years_to_event"].predict(input_df)[0])
        risk = float(models["cvd_risk_pct"].predict(input_df)[0])
        category = str(models["risk_category"].predict(input_df)[0])
        ...
        if activity and any(
            x in activity for x in ["low", "sedentary", "none", "inactive"]
        ):
            suggestions.append(
        ...
        # ------------------------------------------------------------
        # AI Clinical Summary (Groq) — one auto-generated plain-language
        # paragraph explaining this specific result. Cached per prediction
        # so it isn't re-requested on every rerun/interaction.
        # ------------------------------------------------------------
        ai_language = get_ai_language()
        groq_client = get_groq_client()

        if groq_client is None:
            st.info(
                "Add a GROQ_API_KEY (Streamlit secrets or environment "
                "variable) to unlock the AI clinical summary and lifestyle plan."
            )
        else:
            cache_key = f"ai_summary_{ai_language}"
            if cache_key not in st.session_state.prediction:
                with st.spinner("Generating AI summary..."):
                    st.session_state.prediction[cache_key] = generate_ai_summary(
                        st.session_state.prediction, ai_language
                    )

            ai_summary = st.session_state.prediction.get(cache_key)

            if ai_summary and not str(ai_summary).startswith("__ERROR__"):
                st.markdown(
                    f"""
                    <div class="report-card" style="margin-top:18px;">
                        <div class="report-title">✦ AI Clinical Summary</div>
                        <p class="report-text">{ai_summary}</p>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                ...
                st.warning(
                    f"AI summary unavailable: {ai_summary.replace('__ERROR__:', '')}"
                )

            # --------------------------------------------------------
            # AI Lifestyle & Diet Plan — generated on demand (not
            # automatically) since it's a heavier, more detailed call.
            # --------------------------------------------------------
            st.markdown('<div style="height:6px"></div>', unsafe_allow_html=True)

            if st.button(
                "✦ Generate AI Lifestyle & Diet Plan",
                key="gen_lifestyle_plan",
                use_container_width=True,
            ):
                plan_cache_key = f"ai_lifestyle_plan_{ai_language}"
                with st.spinner("Building your personalized lifestyle plan..."):
                    st.session_state.prediction[plan_cache_key] = (
                        generate_lifestyle_plan(
                            st.session_state.prediction, values, ai_language
                        )
                    )

            for lang_option in ["English", "Urdu"]:
                plan_key = f"ai_lifestyle_plan_{lang_option}"
                plan_text = st.session_state.prediction.get(plan_key)
                if plan_text and lang_option == ai_language:
                    if str(plan_text).startswith("__ERROR__"):
                        st.warning(
                            f"Lifestyle plan unavailable: {plan_text.replace('__ERROR__:', '')}"
                        )
                    else:
                        st.markdown(
                            f"""
                            <div class="report-card" style="margin-top:14px;">
                                <div class="report-title">✦ AI Lifestyle & Diet Plan</div>
                                <p class="report-text" style="white-space:pre-line;">{plan_text}</p>
                            </div>
                            """,
                            unsafe_allow_html=True,
                        )
...
def render_ai_assistance():
    st.markdown('<div class="chat-title">AI Assistance</div>', unsafe_allow_html=True)
    st.markdown(
    ...
    if not st.session_state.get("prediction"):
        st.info(
            "Generate a cardiac-risk prediction first. The AI assistant will then have the model results as context."
        )
...
patterns clearly and cautiously. Respond in {ai_language}.
...
"""
        ...
        try:
            ...
            for delta in stream_groq_reply(client, messages):
                full_reply += delta
                placeholder.markdown(
                    f'<div class="chat-bubble-ai">{full_reply}▍</div>',
                    unsafe_allow_html=True,
                )
...
def bmi_calculator():
    ...
    # ---------------- CALCULATE BUTTON ----------------
    if st.button(
        "Calculate BMI", type="primary", use_container_width=True, key="bmi_calc"
    ):
        ...
        if bmi < 18.5:
            ...
            message = "Your BMI is below the healthy weight range."

        elif bmi < 25:
            category = "Normal Weight"
            message = "Your BMI is within the healthy weight range."

        elif bmi < 30:
            category = "Overweight"
            message = "Your BMI is above the healthy weight range."

        else:
            category = "Obesity"
            message = "Your BMI is significantly above the healthy weight range."

        # Save BMI in session state
        st.session_state.calculated_bmi = round(bmi, 2)
        ...
        with result_col1:
            st.metric(label="BMI Score", value=f"{bmi:.2f}")

        with result_col2:
            st.metric(label="BMI Category", value=category)

        # Show numeric BMI clearly
        st.markdown(f"""
        ...
        st.caption(
            "BMI is a screening measurement and does not diagnose health conditions."
        )
  

# ----------------------------- BMI PAGE -----------------------------

if st.session_state.active_page == "Calculate BMI":
    bmi_calculator()
...
def build_report_pdf(prediction):
    ...
    def stat_cards(
        ...
        for title_txt, val, unit, bg in cards:
            ...
            vstyle = cat_value_style if bg else value_style
    ...
    category = str(prediction.get("risk_category") or computed_label)
    ...
    if inputs:
        ...
        input_table.setStyle(TableStyle(style_cmds))
...
def render_report():
    ...
    if not prediction:
        st.info(
            "Generate a prediction first. The report will then contain the model results and possible suggestions."
        )
        return
    ...
    if st.session_state.report_pdf:
        ...
        if st.button("Send PDF to Email", use_container_width=True):
            if not recipient or "@" not in recipient:
                ...
                try:
                    send_report_email(recipient.strip(), st.session_state.report_pdf)
                    st.success(f"Report sent successfully to {recipient.strip()}.")
                except Exception as exc:
...
if st.session_state.active_page == "Prediction":
    ...
    with st.expander("Model information"):
        st.write(f"Training rows: {bundle.get('training_rows', 'N/A')}")
        st.write(f"Model version: {bundle.get('model_version', 'N/A')}")
